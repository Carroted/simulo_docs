...
local component = self:add_component({
    ...
    -- for Advanced Use we can also give a `properties` table, but its not always a good idea since you need to pass everything about the properties, like the name, input_type, value, settings etc
});
```
