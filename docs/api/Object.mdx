...
It's a <Type name="vec2" />. Guess what it is
...
Tries to set the Z index of the object as a <Type name="number" />. Returns false if it failed, true if it won. You should usually use the below functions instead.
...
Gets whether the object is allowed to rotate from physics. You can still `:set_angle` on it even when this is the case.

---

### \:set_angle_locked()

Sets whether the object is allowed to rotate from physics. You can still `:set_angle` on it even when this is the case.
...
Gets a list of all the attachments attached to the object.
